...
class TestDuplicateWorkIntegration:
    """Integration tests for GET /api/eval/duplicate-work."""

    def test_duplicate_work_requires_auth(self, test_db_session: AsyncSession) -> None:
        app, _token = _create_eval_app(test_db_session)
        with TestClient(app) as client:
            response = client.get("/api/eval/duplicate-work")
        assert response.status_code == 401

    def test_duplicate_work_returns_204_when_none_ready(
        self, test_db_session: AsyncSession
    ) -> None:
        app, token = _create_eval_app(test_db_session)
        with TestClient(app) as client:
            response = client.get(
                "/api/eval/duplicate-work",
                headers={"Authorization": f"Bearer {token}"},
            )
        assert response.status_code == 204

    def test_duplicate_work_skips_already_scored(
        self, test_db_session: AsyncSession
    ) -> None:
        project = make_project(id=1, name="snapcraft")
        issue = make_issue(id=1, project_id=1, external_id="42")
        # Evaluation already has duplicateness in scores
        evaluation = make_evaluation(
            id=1,
            issue_id=1,
            summary_embedding=[0.1] * 768,
            scores={"staleness": 5, "duplicateness": 0.0},
        )
        asyncio.get_event_loop().run_until_complete(
    ...
    def test_duplicate_work_returns_and_locks_item(
        self, test_db_session: AsyncSession
    ) -> None:
        project = make_project(id=1, name="snapcraft")
        issue = make_issue(id=1, project_id=1, external_id="42", title="Build fails")
        evaluation = make_evaluation(
            id=1,
            issue_id=1,
            summary="Build failure on core24",
            summary_embedding=[0.1] * 768,
            scores={"staleness": 5},
        )
        asyncio.get_event_loop().run_until_complete(
        ...
        with TestClient(app) as client:
            ...
            # Second call returns 204 because item is now locked
            second = client.get(
                "/api/eval/duplicate-work",
                headers={"Authorization": f"Bearer {token}"},
            )
        ...
        assert len(items) == 1
        assert items[0]["evaluation_id"] == 1
        assert items[0]["issue_id"] == 1
        assert items[0]["external_id"] == "42"
        assert items[0]["project_name"] == "snapcraft"
        assert items[0]["summary"] == "Build failure on core24"
        assert items[0]["embedding"] == [0.1] * 768
        assert second.status_code == 204


class TestDuplicateResultIntegration:
    """Integration tests for POST /api/eval/duplicate-result."""

    def test_submit_duplicate_result_requires_auth(
        self, test_db_session: AsyncSession
    ) -> None:
        app, _token = _create_eval_app(test_db_session)
        with TestClient(app) as client:
    ...
    def test_submit_duplicate_result_stores_no_duplicate(
        self, test_db_session: AsyncSession
    ) -> None:
        project = make_project(id=1, name="snapcraft")
        issue = make_issue(id=1, project_id=1, external_id="42")
        evaluation = make_evaluation(
            ...
            summary_embedding=[0.1] * 768,
            scores={"staleness": 5},
            duplicate_locked_until=datetime.now(tz=UTC) + timedelta(minutes=10),
        )
        asyncio.get_event_loop().run_until_complete(
            _seed_entities(test_db_session, project, issue, evaluation)
        )
        ...
        assert response.status_code == 200
        assert response.json() == {"status": "stored", "evaluation_id": 1}

    def test_submit_duplicate_result_stores_duplicate_found(
        self, test_db_session: AsyncSession
    ) -> None:
        project = make_project(id=1, name="snapcraft")
        issue = make_issue(id=1, project_id=1, external_id="42")
        ...
        with TestClient(app) as client:
            response = client.post(
                ...
                json={
                    ...
                    "duplicate_of_issue_id": 99,
                    "updated_summary": "May duplicate of snapcraft#99. Build failure on core24.",
                },
            )
        ...
        assert len(stored) == 1
        assert stored[0].scores["duplicateness"] == 87.0
        assert stored[0].scores["staleness"] == 5  # preserved
        assert stored[0].candidates_compared == 2
        assert stored[0].duplicate_of_issue_id == 99
        assert "May duplicate" in stored[0].summary

    def test_submit_duplicate_result_rejects_stale_evaluation(
        self, test_db_session: AsyncSession
    ) -> None:
        project = make_project(id=1, name="snapcraft")
        issue = make_issue(id=1, project_id=1, external_id="42")
        # latest=False simulates phase 1 having re-run
        evaluation = make_evaluation(id=1, issue_id=1, latest=False)
        asyncio.get_event_loop().run_until_complete(
            _seed_entities(test_db_session, project, issue, evaluation)
        )
        app, token = _create_eval_app(test_db_session)
