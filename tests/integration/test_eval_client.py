...
SAMPLE_EVALUATE_RESULT: dict[str, Any] = {
    ...
    "issue_data_hash": "sample_eval_hash_abc123",
}
...
class TestEvalClientHappyPath:
    ...
    async def test_processes_multiple_real_issues_in_sequence(
        ...
        evaluate_results = [
            SAMPLE_EVALUATE_RESULT,
            {
                **SAMPLE_EVALUATE_RESULT,
                ...
                "issue_data_hash": "hash_landscape",
            },
            {
                **SAMPLE_EVALUATE_RESULT,
                "suggested_action": "close_stale",
                "issue_data_hash": "hash_charm",
            },
        ]
        patched_runtime["evaluator"].evaluate.side_effect = evaluate_results
        ...
        assert (
            patched_runtime["evaluator"].evaluate.call_args_list[0].kwargs["title"]
            == issues[0]["title"]
        )
        assert (
            patched_runtime["evaluator"].evaluate.call_args_list[1].kwargs["title"]
            == issues[1]["title"]
        )
        assert (
            patched_runtime["evaluator"].evaluate.call_args_list[2].kwargs["title"]
            == issues[2]["title"]
        )

        # Verify each project name was extracted correctly
        calls = patched_runtime["evaluator"].evaluate.call_args_list
    ...
    async def test_evaluator_exception_is_logged_and_continues(
        ...
        patched_runtime["evaluator"].evaluate.side_effect = [
...
class TestEvalClientSubmissionPayload:
    ...
    async def test_submission_includes_all_real_issue_fields(
        ...
        # Content hash comes from the evaluator's result (issue_data_hash)
        assert submit_json["content_hash"] == SAMPLE_EVALUATE_RESULT["issue_data_hash"]

        # Evaluator result fields
        assert submit_json["summary"] == SAMPLE_EVALUATE_RESULT["summary"]
        assert submit_json["scores"] == SAMPLE_EVALUATE_RESULT["scores"]
        assert (
            submit_json["suggested_action"]
            == SAMPLE_EVALUATE_RESULT["suggested_action"]
        )
        assert (
            submit_json["suggested_action_reason"]
            == SAMPLE_EVALUATE_RESULT["suggested_action_reason"]
        )

        # Token fields from evaluate() result
    ...
    async def test_submission_uses_evaluator_hash_not_server_hash(
        ...
        """The content_hash comes from the evaluator result, not from the server's current_hash."""
        client_mock = _patch_http(
            monkeypatch,
            get_responses=_with_status(
                httpx.Response(status_code=200, json=REAL_ISSUE_SNAPCRAFT)
...
class TestEvalClientEdgeCases:
    ...
    async def test_age_and_activity_days_computed_from_dates(
        ...
        expected_age = (datetime.now(tz=UTC) - created_at).days
