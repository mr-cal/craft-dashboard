"""Integration tests proving parity with starcraft-stats trend computations."""
...
def starcraft_get_median_date(dates: list[datetime]) -> datetime:
    """Exact replica of starcraft-stats get_median_date."""
...
def starcraft_get_median_age(
    ...
    """Exact replica of starcraft-stats get_median_age."""
    if dates:
...
class TestStarcraftStatsParity:
    ...
    def test_median_age_matches_starcraft_stats_for_identical_items(
        self,
        issues: list[dict],
        maintainers: set[str],
        bots: set[str] | None,
    ) -> None:
        """Combined median_age must match starcraft-stats for identical inputs."""
    ...
    def test_closed_count_matches_starcraft_stats(
        self,
        issues: list[dict],
    ) -> None:
        """Combined closed count must match starcraft-stats for identical inputs."""
        result = compute_snapshot_counts(
            issues,
    ...
    def test_median_age_computation_precision(self, issues: list[dict]) -> None:
        """Median age precision must match starcraft-stats across edge cases."""
