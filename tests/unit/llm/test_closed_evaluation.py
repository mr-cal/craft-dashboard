...
async def test_closed_issue_uses_summary_only_evaluation() -> None:
    """Closed issues skip scoring and store no action."""
    mock_summary_response = LLMResponse(
        content="Fixed by the core24 migration patch and closed after confirmation.",
        total_tokens=15,
        prompt_tokens=10,
        completion_tokens=5,
        model="summary-model",
    ...
    mock_client.complete = AsyncMock(return_value=mock_summary_response)
    ...
    assert result["summary"] == mock_summary_response.content
...
async def test_state_change_triggers_reevaluation_with_new_hash() -> None:
    ...
    mock_summary_response = LLMResponse(
        content="Merged after review and closed by the landing commit.",
        total_tokens=10,
        prompt_tokens=7,
        completion_tokens=3,
        model="summary-model",
    ...
    open_hash = await evaluator.evaluate_issue(
    ...
    closed_hash = await evaluator.evaluate_issue(
        title="Add support for core24 base",
        body="Please add support for `base: core24`.",
        issue_type="issue",
        state="closed",
