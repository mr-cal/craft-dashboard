...
class TestIssueList:
    ...
    def test_issues_page_includes_combined_column_visibility_picker(self) -> None:
        ...
        assert 'value="actionability"' in response.text
        assert 'value="complexity"' in response.text
        assert 'value="impact"' in response.text
...
class TestIssueTablePartial:
    ...
    def test_issue_table_shows_closed_and_unevaluated_states(self) -> None:
        ...
        context = {
            ...
            "total_pages": 1,
            "per_page": 100,
            "filter_scores": "impact,actionability",
            "active_scores": ["impact", "actionability"],
            "all_scores": ALL_SCORES,
            "filter_llm_status": "",
            "total_count": 2,
        }
...
class TestRunSemanticSearch:
    ...
    async def test_returns_empty_without_api_key(self) -> None:
        """No embedding API key means no semantic search is attempted."""
        result = await self._run(embedding_api_key="")
        assert result == []
