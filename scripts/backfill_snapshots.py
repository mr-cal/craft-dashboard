...
def compute_snapshot_for_date(
    ...
    open_issues_external = sum(1 for i in open_issues_on_date if not i.author_is_maintainer)
    ...
    # Compute median ages
    issue_ages = [(snapshot_date - i.created_at.date()).days for i in open_issues_on_date]
    ...
    closed_issues_external = sum(1 for i in closed_issues_on_date if not i.author_is_maintainer)
    closed_issues_internal = sum(1 for i in closed_issues_on_date if i.author_is_maintainer)
    closed_prs_external = sum(1 for i in closed_prs_on_date if not i.author_is_maintainer)
    closed_prs_internal = sum(1 for i in closed_prs_on_date if i.author_is_maintainer)
    
    # Count bugs (issues with "bug" label)
    open_bugs = sum(
        1 for i in open_issues_on_date
        if isinstance(i.labels, dict) and "bug" in [l.lower() for l in i.labels.get("labels", [])]
    )
...
def backfill_project(session: Session, project: Project) -> None:
    ...
    print(f"  Date range: {earliest_date} to {today} ({(today - earliest_date).days} days)")
...
def main() -> None:
    ...
    with Session(engine) as session:
        # Fetch all projects
        result = session.execute(
            select(Project).order_by(Project.display_order)
        )
